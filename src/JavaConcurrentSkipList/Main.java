...
/*    private static Runnable ContainsOperation(SkipListKey.OperationType operationType, final int operationValue, final ConcurrentSkipListSet<Integer> skiplist) {
        return new Runnable() {
            public void run() {
                try{
                    skiplist.contains(operationValue);
                }catch( Exception ex) {
